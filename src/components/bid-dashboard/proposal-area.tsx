...
const PAID_STATUSES: BidStatus[] = ["in_review", "submitted", "won"];
...
export function ProposalArea({ executiveSummary, fullProposal, bidStatus, onExportClick }: ProposalAreaProps) {
  const isGenerating = !executiveSummary && !fullProposal;
  const isPaid = PAID_STATUSES.includes(bidStatus);
